...
/*
 * ------------------------------------------------
 * TRANSCLUSION-AWARE HOST RESOLUTION
 * ------------------------------------------------
 *
 * Digital Garden's Obsidian plugin expands:
 *
 *     ![[Page A]]
 *
 * by copying Page A's markdown directly into the containing page,
 * wrapped in:
 *
 *     <div class="transclusion internal-embed is-loaded">
 *
 * If Page A is published, that wrapper also contains:
 *
 *     <a class="markdown-embed-link" href="/page-a-url">
 *
 * That href is our surviving identity marker for the embedded note.
 */


/*
 * Normalize a rendered garden URL so:
 *
 *     /notes/foo/
 *     /notes/foo
 *     /notes/foo/#Heading
 ...
 */
function normalizeGardenUrl(value) {
  if (!value) {
    return "";
  }
  ...
  try {
    /*
     * Handles both site-relative and absolute URLs.
     */
    result = new URL(
      result,
      "https://digital-garden.invalid"
    ).pathname;
  } catch {
    /*
  ...
  try {
    ...
    /*
     * Leave malformed encoding untouched.
     */
  }
...
/*
 * Read one HTML attribute from an opening tag.
 */
...
/*
 * Determine whether an HTML token opens one of Digital Garden's
 * generated note-transclusion wrappers.
 *
 * If it does, also recover the embedded note's href when available.
 */
function getTransclusionOpenInfo(html) {
  if (!html) {
    return null;
  }
  ...
  /*
   * Match the wrapper generated by GardenPageCompiler, rather than
   * every arbitrary HTML element with a "transclusion" class.
   */
  if (
    !classes.includes("transclusion") ||
    !classes.includes("internal-embed") ||
    !classes.includes("is-loaded")
  ) {
    return null;
  }
  ...
  /*
   * A published embedded note gets a markdown-embed-link pointing
   * back to its own garden URL.
   */
  const anchors =
    String(html).match(
      /<a\b[^>]*>/gi
...
/*
 * GardenPageCompiler closes each generated note transclusion with
 * exactly the two divs it opened:
 *
 *     </div></div>
 */
function isTransclusionCloseToken(html) {
  if (!html) {
    return false;
  }
...
/*
 * Match a transclusion href back to Digital Garden's canonical Bases
 * note object.
 */
function findNoteByGardenUrl(
  href,
  env
) {
  if (!href) {
...
/*
 * Walk the Markdown token stream up to the current Base.
 *
 * Because note transclusions can themselves contain transclusions,
 * maintain a stack:
 *
 *     Page B
 *       Page A
 *         Page C
 *
 * A Base always belongs to the nearest currently-open transclusion.
 */
function findEnclosingTransclusion(
  tokens,
  idx,
  env
) {
  ...
  for (
    ...
    /*
     * End of the most recently opened note transclusion.
     */
    if (
      isTransclusionCloseToken(html)
    ) {
      if (stack.length > 0) {
        stack.pop();
      }
    ...
    /*
     * Beginning of a note transclusion.
     */
    const openInfo =
      getTransclusionOpenInfo(html);
    ...
    if (openInfo) {
      ...
      /*
       * Push EVEN IF note is null.
       *
       * This is important: an unresolved embedded note must not
       * accidentally fall through to Page B and silently reproduce the
       * wrong-context bug.
       */
      stack.push({
        href: openInfo.href,
 ...
 * ------------------------------------------------
 *
 * Native Obsidian semantics:
 *
 *   - ordinary Base on Page A:
 *         this = Page A
 *
 *   - Base inside Page A transcluded into Page B:
 *         this = Page A
 *
 *   - Base inside Page C transcluded into Page A transcluded into B:
 *         this = Page C
 *
 * Therefore transclusion context takes precedence over the outer
 * Eleventy page.
 */
function getHostContext(
  ...
  /*
   * First ask whether this Base lives inside a transcluded note.
   */
  const embedded =
    findEnclosingTransclusion(
      tokens,
      idx,
      env
    );
  ...
  if (embedded.inside) {
    /*
     * We know we're inside a transclusion, but cannot identify the
     * embedded note.
     *
     * FAIL LOUDLY instead of treating the outer page as `this`.
     */
    if (!embedded.note) {
      if (!embedded.href) {
        return {
          error:
            "This Base is inside an embedded note, but the embedded " +
            "note has no published garden URL. Its `this` context " +
            "cannot be recovered.",
        };
      }
      ...
      return {
        error:
          "This Base is inside an embedded note, but Digital Garden " +
          "could not match the embedded note URL " +
          JSON.stringify(
            embedded.href
          ) +
          " to a published note.",
    ...
    /*
     * This is the key distinction from an ordinary page:
     *
     * DO NOT fall back to env for properties.
     *
     * env belongs to the OUTER Page B.
     */
    return {
      path:
        normalizeNotePath(
  ...
  /*
   * No enclosing transclusion:
   *
   * This is the original, already-working Page A case.
   */
  const path =
    getHostPath(env);
...
function getHostProperty(
  ...
  /*
   * For an ordinary page, env is a valid fallback.
   *
   * For a transcluded page, env belongs to the OUTER note and must
   * never be consulted.
   */
  if (
    host.allowEnvFallback === false
  ) {
    return undefined;
  }
