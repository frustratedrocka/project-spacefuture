...
Blades In The Dark, which popularized the concept of clocks, suggests using clocks with no less than 4 and no more than 12 segments, with 8 as the go-to if you need to just pick a number and move on. Blades makes heavy use of clocks for many of its systems, but the most common and most relevant rules are:
